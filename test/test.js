...
describe("The /metrics endpoint", function () {
  it("GET the correct metrics", function () {
    return chai
      ...
      .then(function (res) {
        ...
        expect(res.body.metrics).to.deep.equal(
          require("../utils/metrics.js").metrics
        );
      });
  });
  ...
  it("POST and validate the correct metrics", function () {
    return chai
      ...
      .post("/metrics")
      .send({
        metrics: require("../utils/metrics.js").metrics.slice(0, 1),
        expression: "X+5=4",
      })
      .then(function (res) {
        expect(res).to.have.status(200);
  ...
  it("POST and validate the incorrect metrics", function () {
    return chai
      .request(app)
      .post("/metrics")
      .send({ metrics: ["fdgf", "afsfdss"], expression: "X+5=4" })
      .then(function (res) {
